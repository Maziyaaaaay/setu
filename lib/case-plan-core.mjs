/* Setu — case plan generator (shared core).
 *
 * POST /api/case-plan   body: { service, category, portal, notes, purpose, timing }
 *
 * Web-standard handler (Request -> Response). Imported by both:
 *   - api/case-plan.js                (Vercel, edge runtime)
 *   - netlify/functions/case-plan.mjs (Netlify functions)
 *
 * Turns one public-service goal into a structured, plain-language plan via an
 * OpenAI model, grounded in `notes` (from catalog.js). Reads OPENAI_API_KEY and
 * optional OPENAI_BASE_URL from the environment. On any failure it returns
 * { fallback: true } and the frontend renders a bundled generic plan, so the
