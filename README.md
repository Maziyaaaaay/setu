...
**Live: https://setu-blond-eight.vercel.app** (backup: https://setu-india.netlify.app)
...
browser (site/index.html + site/catalog.js + site/app.js)
        ...
        ▼
edge function  — lib/case-plan-core.mjs (shared)
   entry: api/case-plan.js (Vercel) · netlify/functions/case-plan.mjs (Netlify)
...
- If the function is unreachable, the frontend renders a bundled fallback plan,
  tagged "backup plan" so it is never mistaken for the live output.
...
npx vercel dev        # needs OPENAI_API_KEY exported
npx netlify-cli dev   # uses the Netlify AI gateway if logged in
```

## Deploy

- **Vercel (primary):** `git push` — auto-deploys. `OPENAI_API_KEY` is set in the
  Vercel project.
- **Netlify (backup):** `./deploy.sh`.

See `DEPLOY.md` for the full picture (incl. the Vercel Deployment-Protection toggle).
